...
BaseBK001x = function()
{
  ...
  function updateVideoTiming() {
    if (is11M) {
      ...
      // 50 строк верхнего гашения = 12800 тактов (калибровка для идеального попадания
      // переключения палитр в межстрочные черные промежутки).
      CYCLES_PER_LINE = 256;
      VBLANK_LINES = 50;
      VBLANK_CYCLES = 50 * 256; // 12800 тактов
    } else {
      // БК-0010: тактовая частота ЦП 3 МГц, 192 такта на строку.
      CYCLES_PER_LINE = 192;
      VBLANK_LINES = 50;
      VBLANK_CYCLES = 50 * 192; // 9600 тактов
    }
  }
