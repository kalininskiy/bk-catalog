...
Включает профессиональный редактор кода (Monaco Editor), два встроенных кросс-компилятора (BKTurbo8 C++ WASM и PDPy11 Python WASM), языковой сервер (LSP), моментальный запуск в эмуляторе БК и 100% автономную работу без подключения к сети.
...
│   ├── compiler-bridge.js      # Единый мост переключения компиляторов (BKTurbo8 / PDPy11)
...
- ✅ **Два встроенных компилятора (Client-side WASM, без серверной части):**
  - **BKTurbo8 (C++ WASM)** — молниеносная скорость компиляции классического синтаксиса MACRO-11 и Turbo8.
...
- ✅ **Интеллектуальное автоопределение компилятора:**
  - При открытии любого проекта или распаковке ZIP-архива IDE анализирует код и автоматически выбирает нужный транслятор (PDPy11 или BKTurbo8).
...
- ✅ **Сохранение и экспорт всех артефактов:**
  - Все выходные файлы компиляции (`.bin`, `.raw`, `.wav`, `.lst`) сохраняются в проекте, отображаются в дереве файлов и включаются в скачиваемый `.ZIP`.
...
| Параметр               | BKTurbo8 (C++ WASM)                                                       | PDPy11 (Python WASM)                                                                     |
| ---------------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| **Движок**             | Скомпилирован в WebAssembly из C++                                        | CPython 3.12 в WebAssembly (Pyodide)                                                     |
| **Скорость сборки**    | Сверхбыстрая (~10-30 мс)                                                  | Быстрая (~200-900 мс)                                                                    |
| **Синтаксис**          | Классический PDP-11 + расширения BKTurbo8 (.script, .ends, @include, %0=) | Классический PDP-11 + расширения PDPy11 (.repeat, .extern, .dword, .charset)             |
| **Команды сборки**     | Параметры командной строки (`-s`, `-r`, `-x`, `-l`)                       | Метакоманды в коде (`make_bin`, `make_raw`, `make_wav`, `make_turbo_wav`, `insert_file`) |
| **Блоки кода**         | `.REPT ... .ENDR`                                                         | `.REPT` и `.REPEAT <N> { ... }` (в фигурных скобках)                                     |
| **Связывание модулей** | `.GLOBL`                                                                  | `.GLOBL`, `.EXTERN`, `.EXTERN ALL`                                                       |
...
- После сборки проекта все сгенерированные файлы (`.bin`, `.lst`, `.raw`, `.wav`) автоматически появляются в списке файлов проекта слева.
- Нажмите зелёную кнопку **`▶`** рядом с любым `.bin`-файлом, чтобы немедленно запустить его в окне виртуальной БК.
- Нажатие на файл `.lst` открывает полный ассемблерный листинг для изучения сформированного машинного кода.

### 4. Экспорт и обмен проектами
